...
#
# ------
# n2s/scripts/pbnas_blob_worker_ts.py

"""
Optimized blob worker using TABLESAMPLE for multi-worker scalability.

Key improvements:
- Uses TABLESAMPLE for extremely fast random sampling
- No shared candidate pool = no worker contention
- O(1) claim complexity regardless of table size
- Each worker gets different random subset automatically
- Consistent fast claim times even with many workers
"""
...
def get_claim_connection():
    ...
    try:
        if _claim_connection is None or _claim_connection.closed:
            logger.debug("Creating NEW claim connection")
            _claim_connection = get_db_connection()
        else:
            logger.debug("Reusing existing claim connection")
    except (psycopg2.Error, AttributeError):
        # Connection is dead, create new one
        logger.debug("Connection dead, creating new one")
        _claim_connection = get_db_connection()
...
def claim_work() -> Optional[str]:
    ...
    try:
        ...
        with claim_conn.cursor() as cur:
            ...
            # Use TABLESAMPLE for fast random sampling without contention
            # Each worker gets a different random sample automatically
            # Increased to 1% for better hit rate with multiple workers
            cur.execute("""
                UPDATE fs
                SET processing_started = NOW()
                WHERE pth = (
                  SELECT pth
                  FROM fs TABLESAMPLE BERNOULLI(1.0)  -- Sample 1% of table (~12k rows)
                  WHERE main = true
                    AND blobid IS NULL
                ...
                AND processing_started IS NULL  -- Double-check to prevent race condition
                RETURNING pth
            """)
...
def main():
    ...
    setup_logging()
    logger.info("Starting pbnas_blob_worker_ts (TABLESAMPLE version for multi-worker scalability)")
